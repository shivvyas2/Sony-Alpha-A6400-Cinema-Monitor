    ...
    public static func document(pairs: [TakePair], projectFPS: Int, eventName: String) -> String {
        ...
        for p in pairs {
            ...
            if case .exported(let wav) = p.status {
                ...
                resources.append("<asset id=\"\(wavID)\" name=\"\(wavName)\" start=\"0s\" duration=\"\(dur)\" hasAudio=\"1\" audioSources=\"1\" audioChannels=\"\(wavChannels)\" audioRate=\"\(wavRate)\"><media-rep kind=\"original-media\" src=\"\(fileURL(wav))\"/></asset>")
                items.append("""
